...
export function deriveOnboardingFacts(answers: Required<OnboardingAnswers>): DerivedOnboardingFact[] {
  ...
  return [
    fact(answers, "housing", {
      ...
      value: {
        housing_scope:
          answers.housing === "shared_home" || answers.housing === "family_home"
            ? "shared_decisions"
    ...
    fact(answers, "free_time", {
      ...
      value: {
        ...
        suggested_effort_minutes:
          freeTime === "micro" ? 5 : freeTime === "short" ? 15 : freeTime === "medium" ? 30 : 45,
      },
    }),
    fact(answers, "financial_friction", {
      ...
      value: {
        ...
        mission_guidance:
          finance === "high" ? "prefer_free_missions" : "low_cost_missions_allowed_when_relevant",
      },
    }),
    fact(answers, "dietary_restrictions", {
      ...
      value: {
        ...
        mission_guidance:
          diet === "none" ? "standard_food_missions_allowed" : "avoid_assuming_specific_food_swaps",
      },
    }),
    ...
    fact(answers, "sustainability_experience", {
      ...
      fact_type:
        answers.sustainability_experience === "some_habits" ||
...
export function buildOnboardingLearnedPreferences(
  ...
  const interests: string[] = [`Objetivo pessoal: ${labelFor("personal_goal", answers.personal_goal)}`];
  ...
  if (answers.reuse_storage_space !== "none")
    interests.push("Alguma abertura para reutilização e organização");
  ...
  if (answers.reuse_storage_space === "none")
    hardBlocks.push("Evitar missões que precisem guardar muitos itens");
  if (answers.financial_friction === "high") hardBlocks.push("Priorizar missões sem gasto");
  if (answers.health_safety_limits !== "none")
    hardBlocks.push("Priorizar missões de baixo risco físico e logístico");
  if (answers.dietary_restrictions !== "none")
    hardBlocks.push("Respeitar restrições alimentares sem pedir detalhes sensíveis");

  if (answers.free_time === "micro") deficits.push("Precisa de missões de até 5 minutos");
  if (answers.work_study_routine === "intense")
    deficits.push("Rotina intensa pede ações simples e previsíveis");
  if (answers.sustainability_experience === "starting")
    deficits.push("Está começando e precisa de instruções curtas");
  if (answers.sustainability_experience === "frustrated")
    deficits.push("Já teve dificuldade de manter hábitos e precisa de baixa fricção");
  if (answers.change_readiness === "observe_first")
    deficits.push("Prefere observar antes de assumir mudanças maiores");
