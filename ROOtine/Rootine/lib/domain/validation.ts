...
import { hasPositiveImpact, IMPACT_METRIC_KEYS, isImpactMetricKey, type ImpactEstimate } from "./impact";
...
export function validateMissionPersonalization(
  ...
  const usedFactKeys = Array.isArray(candidate.used_fact_keys)
    ? candidate.used_fact_keys.filter((item): item is string => typeof item === "string" && item.trim().length > 0)
    : [];
  ...
  if (hasText(candidate.title) && hasText(candidate.description) && options.recentMissionTexts?.length) {
    ...
    const similarRecent = options.recentMissionTexts.some((recent) => similarityScore(missionText, recent) >= 0.72);
    if (similarRecent) {
...
export function validateFeedbackClassification(feedback: unknown): ValidationResult {
  ...
  if (
    (feedback.kind === "constraint" || feedback.kind === "hard_block" || feedback.creates_hard_block === true) &&
    !hasText(feedback.fact_key, 6)
  ) {
    errors.push("feedback_fact_key_required_for_constraint");
  }

  if (hasText(feedback.raw_text_summary) && feedback.raw_text_summary.length > 280) {
...
export { getLevelFromXp, getMissionXpReward };
